...
#include "rtwtypes.h"
#include "Foc_model_Matlab_1_private.h"
#include <math.h>
#include "rt_nonfinite.h"
#include <float.h>

/* Named constants for Chart: '<S4>/Enable PDB and start FTM' */
#define Foc_model_Matlab_1_IN_A        ((uint8_T)1U)
#define Foc_model_Matlab_1_IN_END      ((uint8_T)2U)
...
volatile real32_T K2;                  /* '<Root>/Data Store Memory16' */
volatile real32_T Ki;                  /* '<Root>/Data Store Memory15' */
volatile real32_T Kp;                  /* '<Root>/Data Store Memory14' */
...
B_Foc_model_Matlab_1_T Foc_model_Matlab_1_B;

/* Block states (default storage) */
DW_Foc_model_Matlab_1_T Foc_model_Matlab_1_DW;

/* Real-time model */
static RT_MODEL_Foc_model_Matlab_1_T Foc_model_Matlab_1_M_;
RT_MODEL_Foc_model_Matlab_1_T *const Foc_model_Matlab_1_M =
  &Foc_model_Matlab_1_M_;
...
void ADC1_SC1reg2U_callback(void)
{
  ...
  Foc_model_Matlab_1_B.ADC1_ISR_o2 = result;
  ADC_DRV_GetChanConfig(1, 2U, &config);
  Foc_model_Matlab_1_B.ADC1_ISR_o3 = config.channel;
  ...
  /* user code (Output function Body) */
  {
    ...
    ADC_IB = Foc_model_Matlab_1_B.ADC1_ISR_o2;
 ...
 */
void Foc_model_Matlab_1_SetEventsForThisBaseStep(boolean_T *eventFlags)
{
  /* Task runs when its counter is zero, computed via rtmStepTask macro */
  eventFlags[1] = ((boolean_T)rtmStepTask(Foc_model_Matlab_1_M, 1));
  eventFlags[2] = ((boolean_T)rtmStepTask(Foc_model_Matlab_1_M, 2));
  eventFlags[3] = ((boolean_T)rtmStepTask(Foc_model_Matlab_1_M, 3));
}
...
static void rate_monotonic_scheduler(void)
{
   ...
   */
  (Foc_model_Matlab_1_M->Timing.TaskCounters.TID[1])++;
  if ((Foc_model_Matlab_1_M->Timing.TaskCounters.TID[1]) > 1) {/* Sample time: [0.0001s, 0.0s] */
    Foc_model_Matlab_1_M->Timing.TaskCounters.TID[1] = 0;
  }

  (Foc_model_Matlab_1_M->Timing.TaskCounters.TID[2])++;
  if ((Foc_model_Matlab_1_M->Timing.TaskCounters.TID[2]) > 19) {/* Sample time: [0.001s, 0.0s] */
    Foc_model_Matlab_1_M->Timing.TaskCounters.TID[2] = 0;
  }

  (Foc_model_Matlab_1_M->Timing.TaskCounters.TID[3])++;
  if ((Foc_model_Matlab_1_M->Timing.TaskCounters.TID[3]) > 1999) {/* Sample time: [0.1s, 0.0s] */
    Foc_model_Matlab_1_M->Timing.TaskCounters.TID[3] = 0;
  }
}
...
void Foc_mod_CurrentControl_Init(void)
{
  ...
  Foc_model_Matlab_1_DW.DelayOneStep1_DSTATE = true;

  /* InitializeConditions for Delay: '<S79>/Delay One Step' */
  Foc_model_Matlab_1_DW.DelayOneStep_DSTATE = 500U;
...
void Foc_model_Ma_CurrentControl(void)
{
  ...
  real32_T rtb_Numberofpolepairs;
  ...
  /* user code (Output function Body) */
  {
    ...
    /* Start Profiling This Function.*/
    if (Foc_model_Matlab_1_DW.CurrentControl_RESET_ELAPS_T) {
      CurrentControl_ELAPS_T = 0U;
    } else {
      CurrentControl_ELAPS_T = Foc_model_Matlab_1_M->Timing.clockTick1 -
        Foc_model_Matlab_1_DW.CurrentControl_PREV_T;
    }

    Foc_model_Matlab_1_DW.CurrentControl_PREV_T =
      Foc_model_Matlab_1_M->Timing.clockTick1;
    Foc_model_Matlab_1_DW.CurrentControl_RESET_ELAPS_T = false;
    ...
    Foc_model_Matlab_1_B.Digital_Input_HALL_C = (PINS_DRV_ReadPins(PTA) >> 1) &
      0x01;
    ...
    /* '<S76>:1:6' */
    HALL_C_controller = (uint32_T)Foc_model_Matlab_1_B.Digital_Input_HALL_C << 2;
    ...
    Foc_model_Matlab_1_B.Digital_Input_HALL_B = (PINS_DRV_ReadPins(PTD) >> 10) &
      0x01;
    ...
    /* '<S77>:1:6' */
    HALL_B_controller = (uint32_T)Foc_model_Matlab_1_B.Digital_Input_HALL_B << 1;
    ...
    Foc_model_Matlab_1_B.Digital_Input_HALL_A = (PINS_DRV_ReadPins(PTD) >> 11) &
      0x01;

    /* DataTypeConversion: '<S73>/Data Type Conversion6' */
    HALL_A_controller = (uint32_T)Foc_model_Matlab_1_B.Digital_Input_HALL_A;
     ...
     */
    OR = (Foc_model_Matlab_1_DW.DelayOneStep1_DSTATE || (rtb_DataStoreRead5 != 0));
    ...
    if (OR) {
       ...
       *  DataTypeConversion: '<S71>/Data Type Conversion4'
       */
      Foc_model_Matlab_1_B.DelayOneStep =
        Foc_model_Matlab_1_DW.DelayOneStep_DSTATE;
    }
     ...
     */
    rtb_Compare = (Foc_model_Matlab_1_B.DelayOneStep > 0);
    ...
    if (rtb_Compare) {
      ...
      if (rtb_DataStoreRead5 >= 31250) {
        ...
        rtb_DataStoreRead5 = (uint16_T)((GlobalSpeedValidity != 0) ||
          Foc_model_Matlab_1_B.validityDelay);
      }
    ...
    if (rtb_DataStoreRead5 != 0) {
      ...
      if (GlobalDirection > 0) {
         ...
         *  Product: '<S81>/Divide'
         */
        rtb_Merge_ik = Foc_model_Matlab_1_ConstB.SpeedConstData / (real32_T)
      ...
      if (GlobalDirection != 1) {
         ...
         *  Sum: '<S96>/Sum'
         */
        Foc_model_Matlab_1_B.Merge = rtb_Merge1_p - rtb_Merge1;
       ...
       */
      ThetaHalls = Foc_model_Matlab_1_B.Merge;
    ...
    /* Sum: '<S79>/Sum' */
    rtb_DataStoreRead5 = Foc_model_Matlab_1_B.DelayOneStep;
    ...
    if (ThetaHalls <= 0.7061F) {
       ...
       */
      rtb_Gain_b = (ThetaHalls + 1.0F) - 0.7061F;
     ...
     *  Rounding: '<S116>/Floor'
     */
    Pos_PU = rtb_Gain_b - (real32_T)floor(rtb_Gain_b);
     ...
     *  Sum: '<S24>/a_plus_2b'
     */
    rtb_DTC = ((Iab_fb[0] + Iab_fb[1]) + Iab_fb[1]) * 0.577350259F;
    ...
    if (Foc_model_Matlab_1_DW.DiscreteTimeIntegrator4_SYSTEM_ != 0) {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator4' */
      rtb_Merge1 = Foc_model_Matlab_1_DW.DiscreteTimeIntegrator4_DSTATE;
    } else {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator4' */
      rtb_Merge1 = 0.0001F * (real32_T)CurrentControl_ELAPS_T
        * Foc_model_Matlab_1_DW.DiscreteTimeIntegrator4_PREV_U +
        Foc_model_Matlab_1_DW.DiscreteTimeIntegrator4_DSTATE;
    }

    /* End of DiscreteIntegrator: '<S14>/Discrete-Time Integrator4' */

    /* Gain: '<S14>/Gain' */
    rtb_Gain_b = 6.28318548F * rtb_Merge1;

    /* Gain: '<S1>/Get_PU1' incorporates:
     *  Constant: '<S1>/Constant2'
     *  Math: '<S1>/Mod'
     */
    rtb_Merge1_p = 0.0795774683F * rt_modf_snf(rtb_Gain_b, 12.566371F);

    /* If: '<S123>/If' incorporates:
     *  Constant: '<S122>/Constant1'
     *  Gain: '<S128>/Number of pole pairs'
     *  Gain: '<S1>/Get_PU1'
     *  Sum: '<S125>/Add'
     *  Sum: '<S126>/Add'
     *  Switch: '<S122>/Switch'
    ...
    if (rtb_Merge1_p <= 0.0F) {
       ...
       */
      rtb_Max = rtb_Merge1_p;

      /* End of Outputs for SubSystem: '<S123>/If Action Subsystem1' */
    }

    rtb_Numberofpolepairs = 2.0F * rtb_Max;
     ...
     *  Sum: '<S124>/Add'
     */
    rtb_Numberofpolepairs -= (real32_T)floor(rtb_Numberofpolepairs);
     ...
     *  Sum: '<S58>/Sum'
     */
    if (rtb_Numberofpolepairs < 0.0F) {
      /* Outputs for IfAction SubSystem: '<S54>/If Action Subsystem' incorporates:
       *  ActionPort: '<S57>/Action Port'
       */
      rtb_Max = rtb_Numberofpolepairs - (real32_T)floor(rtb_Numberofpolepairs);
       ...
       */
      rtb_Max = rtb_Numberofpolepairs - (real32_T)(int16_T)rtb_Numberofpolepairs;

      /* End of Outputs for SubSystem: '<S54>/If Action Subsystem1' */
    }

    rtb_Add1 = 800.0F * rtb_Max;
     ...
     */
    rtb_Add2 = rtb_Add1 - (real32_T)(uint16_T)rtb_Add1;

    /* Selector: '<S52>/Lookup' incorporates:
     *  Constant: '<S52>/sine_table_values'
     *  DataTypeConversion: '<S52>/Get_Integer'
     */
    rtb_Numberofpolepairs = Foc_model_Matlab_1_ConstP.sine_table_values_Value
      [(uint16_T)rtb_Add1];
     ...
     *  Sum: '<S53>/Sum3'
     */
    SIN = (Foc_model_Matlab_1_ConstP.sine_table_values_Value[(int32_T)((uint16_T)
            rtb_Add1 + 1U)] - rtb_Numberofpolepairs) * rtb_Add2 +
     ...
     */
    rtb_Numberofpolepairs = Foc_model_Matlab_1_ConstP.sine_table_values_Value
      [(int32_T)((uint16_T)rtb_Add1 + 200U)];
     ...
     *  Sum: '<S53>/Sum5'
     */
    COS = (Foc_model_Matlab_1_ConstP.sine_table_values_Value[(int32_T)((uint16_T)
            rtb_Add1 + 201U)] - rtb_Numberofpolepairs) * rtb_Add2 +
      rtb_Numberofpolepairs;
     ...
     *  Sum: '<S50>/sum_Qs'
     */
    Iq_fb = rtb_DTC * COS - Iab_fb[0] * SIN;
    ...
    /* '<S46>:1:4' */
    rtb_Numberofpolepairs = 1.414F * wnq * 0.000435F - 0.636666656F;

    /* '<S46>:1:5' */
    wnq = rtb_Numberofpolepairs / (wnq * wnq * 0.000435F);

    /* '<S46>:1:6' */
    /* '<S46>:1:7' */
    wnq *= rtb_Numberofpolepairs / wnq;
     ...
     */
    if ((!Foc_model_Matlab_1_DW.integral_not_empty) || tmp) {
      /* '<S47>:1:14' */
      /* '<S47>:1:15' */
      Foc_model_Matlab_1_DW.integral = 0.0F;
      Foc_model_Matlab_1_DW.integral_not_empty = true;
    }

    /* '<S47>:1:19' */
    rtb_Numberofpolepairs *= Iq_err;
    rtb_Add2 = wnq * Foc_model_Matlab_1_DW.integral + rtb_Numberofpolepairs;
    if (((rtb_Add2 < 1.0F) && (rtb_Add2 > -1.0F)) || ((rtb_Add2 >= 1.0F) &&
         (Iq_err < 0.0F)) || ((rtb_Add2 <= -1.0F) && (Iq_err > 0.0F))) {
      /* '<S47>:1:22' */
      /* '<S47>:1:23' */
      /* '<S47>:1:24' */
      /* '<S47>:1:25' */
      Foc_model_Matlab_1_DW.integral += Iq_err;
    }

    /* '<S47>:1:29' */
    /* '<S47>:1:32' */
    Vq_ref_beforeLimiter = wnq * Foc_model_Matlab_1_DW.integral +
      rtb_Numberofpolepairs;
     ...
     *  Sum: '<S50>/sum_Ds'
     */
    Id_fb = Iab_fb[0] * COS + rtb_DTC * SIN;
     ...
     */
    wnq = ((real32_T)(0.0079942689836159844 * rtb_Add2) + Vq_ref_beforeLimiter)
    ...
    /* '<S44>:1:3' */
    rtb_Add1 *= 1697.77783F;

    /* '<S44>:1:4' */
    rtb_Numberofpolepairs = 1.414F * rtb_Add1 * 0.000375F - 0.636666656F;

    /* '<S44>:1:5' */
    rtb_Add1 = rtb_Numberofpolepairs / (rtb_Add1 * rtb_Add1 * 0.000375F);

    /* '<S44>:1:6' */
    /* '<S44>:1:7' */
    rtb_Add1 *= rtb_Numberofpolepairs / rtb_Add1;
    ...
    /* '<S45>:1:10' */
    /* '<S45>:1:11' */
    if ((!Foc_model_Matlab_1_DW.integral_not_empty_p) || tmp) {
      /* '<S45>:1:14' */
      /* '<S45>:1:15' */
      Foc_model_Matlab_1_DW.integral_j = 0.0F;
      Foc_model_Matlab_1_DW.integral_not_empty_p = true;
    }

    /* '<S45>:1:19' */
    rtb_Numberofpolepairs *= Id_err;
    rtb_Add2 = rtb_Add1 * Foc_model_Matlab_1_DW.integral_j +
      rtb_Numberofpolepairs;
    if (((rtb_Add2 < 1.0F) && (rtb_Add2 > -1.0F)) || ((rtb_Add2 >= 1.0F) &&
         (Id_err < 0.0F)) || ((rtb_Add2 <= -1.0F) && (Id_err > 0.0F))) {
      /* '<S45>:1:22' */
      /* '<S45>:1:23' */
      /* '<S45>:1:24' */
      /* '<S45>:1:25' */
      Foc_model_Matlab_1_DW.integral_j += Id_err;
    }

    /* '<S45>:1:29' */
    /* '<S45>:1:32' */
    Vd_ref_beforeLimiter = rtb_Add1 * Foc_model_Matlab_1_DW.integral_j +
      rtb_Numberofpolepairs;
     ...
     *  Product: '<S26>/Product'
     */
    rtb_Add1 = Vd_ref_beforeLimiter - 2.0F * rtb_Merge_ik * Iq_fb * 0.000435F;

    /* Sum: '<S33>/Sum1' incorporates:
     *  Product: '<S33>/Product'
     *  Product: '<S33>/Product1'
     */
    rtb_Numberofpolepairs = rtb_Add1 * rtb_Add1 + wnq * wnq;
     ...
     */
    if (rtb_Numberofpolepairs > 0.9025F) {
      /* Outputs for IfAction SubSystem: '<S30>/Limiter' incorporates:
       *  ActionPort: '<S34>/Action Port'
       */
      /* Sqrt: '<S34>/Square Root' */
      rtb_Numberofpolepairs = (real32_T)sqrt(rtb_Numberofpolepairs);
       ...
       *  Switch: '<S34>/Switch'
       */
      rtb_Add1 = rtb_Add1 * 0.95F / rtb_Numberofpolepairs;
      wnq = wnq * 0.95F / rtb_Numberofpolepairs;
     ...
     *  Sum: '<S48>/sum_beta'
     */
    rtb_Numberofpolepairs = rtb_Add1 * COS - wnq * SIN;
    wnq = wnq * COS + rtb_Add1 * SIN;

    /* Gain: '<S66>/one_by_two' incorporates:
     *  AlgorithmDescriptorDelegate generated from: '<S48>/a16'
     */
    rtb_Add2 = 0.5F * rtb_Numberofpolepairs;

    /* Gain: '<S66>/sqrt3_by_two' incorporates:
     *  AlgorithmDescriptorDelegate generated from: '<S48>/a16'
     */
    rtb_Max = 0.866025388F * wnq;

    /* End of Outputs for SubSystem: '<S19>/Two inputs CRL' */

    /* Sum: '<S66>/add_b' */
    rtb_Add1 = rtb_Max - rtb_Add2;

    /* Sum: '<S66>/add_c' */
    rtb_Add2 = (0.0F - rtb_Add2) - rtb_Max;
    ...
    /* Outputs for Atomic SubSystem: '<S19>/Two inputs CRL' */
    if ((rtb_Numberofpolepairs >= rtb_Add1) || tmp) {
      rtb_Max = rtb_Numberofpolepairs;
    } else {
      rtb_Max = rtb_Add1;
    }
     ...
     */
    if ((rtb_Numberofpolepairs <= rtb_Add1) || tmp) {
      int_sat_ib = rtb_Numberofpolepairs;
    } else {
      int_sat_ib = rtb_Add1;
    }
    ...
    if ((!(rtb_Max >= rtb_Add2)) && tmp) {
      rtb_Max = rtb_Add2;
    }

    /* MinMax: '<S63>/Min' */
    if ((!(int_sat_ib <= rtb_Add2)) && tmp) {
      int_sat_ib = rtb_Add2;
    }
     ...
     *  Sum: '<S63>/Add'
     */
    rtb_Max = (rtb_Max + int_sat_ib) * -0.5F;
     ...
     *  Sum: '<S62>/Add3'
     */
    PWM_Duty_Cycles[0] = ((rtb_Numberofpolepairs + rtb_Max) * 1.15470052F + 1.0F)
      * 0.5F;

    /* End of Outputs for SubSystem: '<S19>/Two inputs CRL' */
    PWM_Duty_Cycles[1] = ((rtb_Add1 + rtb_Max) * 1.15470052F + 1.0F) * 0.5F;
    PWM_Duty_Cycles[2] = ((rtb_Max + rtb_Add2) * 1.15470052F + 1.0F) * 0.5F;
     ...
     *  Step: '<S1>/Step1'
     */
    rtb_Step = !(((Foc_model_Matlab_1_M->Timing.clockTick1) * 0.0001) < 0.0);

    /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator2' */
    if (Foc_model_Matlab_1_DW.DiscreteTimeIntegrator2_SYSTEM_ != 0) {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator2' */
      rtb_Merge_ik = Foc_model_Matlab_1_DW.DiscreteTimeIntegrator2_DSTATE;
    } else {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator2' */
      rtb_Merge_ik = 0.0001F * (real32_T)CurrentControl_ELAPS_T
        * Foc_model_Matlab_1_DW.DiscreteTimeIntegrator2_PREV_U +
        Foc_model_Matlab_1_DW.DiscreteTimeIntegrator2_DSTATE;
    }

    /* End of DiscreteIntegrator: '<S14>/Discrete-Time Integrator2' */

    /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator3' */
    if (Foc_model_Matlab_1_DW.DiscreteTimeIntegrator3_SYSTEM_ != 0) {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator3' */
      rtb_Add1 = Foc_model_Matlab_1_DW.DiscreteTimeIntegrator3_DSTATE;
    } else {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator3' */
      rtb_Add1 = 0.0001F * (real32_T)CurrentControl_ELAPS_T
        * Foc_model_Matlab_1_DW.DiscreteTimeIntegrator3_PREV_U +
        Foc_model_Matlab_1_DW.DiscreteTimeIntegrator3_DSTATE;
    }
    ...
    /* '<S131>:1:7' */
    rtb_Add2 = rtb_Merge_ik - (real32_T)(rtb_algDD_o1_f * (real_T)rtb_Step);

    /* Outputs for Atomic SubSystem: '<S23>/Two phase CRL wrap' */
    /* '<S131>:1:8' */
    rtb_algDD_o1_f = rtb_Add1 - (real32_T)(rtb_DTC * (real_T)rtb_Step);

    /* End of Outputs for SubSystem: '<S23>/Two phase CRL wrap' */
    rtb_Max = (real32_T)fabs(rtb_Add2);
    if (rtb_Max > 0.0001) {
      /* '<S131>:1:10' */
      /* '<S131>:1:11' */
      if (rtIsNaNF(rtb_Add2)) {
        ...
        rtb_DTC = (real32_T)(rtb_Add2 > 0.0F);
      }
    } else {
      /* '<S131>:1:13' */
      rtb_DTC = rtb_Max / 0.0001F;
    }

    z_beta = (real32_T)fabs(rtb_algDD_o1_f);
    if (z_beta > 0.0001) {
      /* '<S131>:1:16' */
      /* '<S131>:1:17' */
      ...
      /* '<S131>:1:19' */
      rtb_algDD_o1_f = z_beta / 0.0001F;
    }

    /* End of MATLAB Function: '<S14>/Saturation ' */

    /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator1' */
    if (Foc_model_Matlab_1_DW.DiscreteTimeIntegrator1_SYSTEM_ != 0) {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator1' */
      rtb_Add2 = Foc_model_Matlab_1_DW.DiscreteTimeIntegrator1_DSTATE;
    } else {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator1' */
      rtb_Add2 = 0.0001F * (real32_T)CurrentControl_ELAPS_T
        * Foc_model_Matlab_1_DW.DiscreteTimeIntegrator1_PREV_U +
        Foc_model_Matlab_1_DW.DiscreteTimeIntegrator1_DSTATE;
    }

    /* End of DiscreteIntegrator: '<S14>/Discrete-Time Integrator1' */

    /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator' */
    if (Foc_model_Matlab_1_DW.DiscreteTimeIntegrator_SYSTEM_E != 0) {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator' */
      int_sat_ib = Foc_model_Matlab_1_DW.DiscreteTimeIntegrator_DSTATE;
    } else {
      /* DiscreteIntegrator: '<S14>/Discrete-Time Integrator' */
      int_sat_ib = 0.0001F * (real32_T)CurrentControl_ELAPS_T
        * Foc_model_Matlab_1_DW.DiscreteTimeIntegrator_PREV_U +
        Foc_model_Matlab_1_DW.DiscreteTimeIntegrator_DSTATE;
    }

    /* End of DiscreteIntegrator: '<S14>/Discrete-Time Integrator' */

    /* MATLAB Function: '<S14>/Back-EMF Estimation' incorporates:
     *  DataStoreRead: '<S14>/Data Store Read2'
     */
    /* MATLAB Function 'CurrentControl/STSMO/Back-EMF Estimation': '<S129>:1' */
    /* '<S129>:1:9' */
    z_alpha = Foc_model_Matlab_1_ConstB.CastToSingle6 * (real32_T)sqrt(rtb_Max) *
      rtb_DTC + K2 * rtb_Add2;

    /* '<S129>:1:10' */
    z_beta = Foc_model_Matlab_1_ConstB.CastToSingle6 * (real32_T)sqrt(z_beta) *
      rtb_algDD_o1_f + K2 * int_sat_ib;
    ...
    /* '<S129>:1:15' */
    /* '<S129>:1:16' */
    rtb_Max = (0.0F - z_alpha * (real32_T)cos(rtb_Gain_b)) - z_beta * (real32_T)
      sin(rtb_Gain_b);

    /* Product: '<S163>/IProd Out' incorporates:
     *  DataStoreRead: '<S14>/Data Store Read1'
     */
    rtb_Gain_b = rtb_Max * Ki;

    /* DiscreteIntegrator: '<S166>/Integrator' */
    if (Foc_model_Matlab_1_DW.Integrator_SYSTEM_ENABLE != 0) {
      /* DiscreteIntegrator: '<S166>/Integrator' */
      Integrator = Foc_model_Matlab_1_DW.Integrator_DSTATE;
    } else {
      /* DiscreteIntegrator: '<S166>/Integrator' */
      Integrator = 0.0001F * (real32_T)CurrentControl_ELAPS_T
        * Foc_model_Matlab_1_DW.Integrator_PREV_U +
        Foc_model_Matlab_1_DW.Integrator_DSTATE;
    }

    /* End of DiscreteIntegrator: '<S166>/Integrator' */

    /* Product: '<S171>/PProd Out' incorporates:
     ...
     *  Gain: '<S1>/Get_PU1'
     */
    CurrentControl_ELAPS_T = (uint32_T)(4.2949673E+9F * rtb_Merge1_p);
     ...
     *  Sum: '<S16>/SpeedCount'
     */
    Foc_model_Matlab_1_B.SpeedGain = (real32_T)((int32_T)CurrentControl_ELAPS_T
      - (int32_T)
      Foc_model_Matlab_1_DW.Delay_DSTATE[Foc_model_Matlab_1_DW.CircBufIdx]) *
      6.98491931E-9F;

    /* Update for Delay: '<S79>/Delay One Step1' */
    Foc_model_Matlab_1_DW.DelayOneStep1_DSTATE = rtb_Compare;
     ...
     */
    if (OR) {
      Foc_model_Matlab_1_DW.DelayOneStep_DSTATE = (uint16_T)(rtb_DataStoreRead5
        - 1);
    }

    /* End of Update for Delay: '<S79>/Delay One Step' */

    /* Update for DiscreteIntegrator: '<S14>/Discrete-Time Integrator4' incorporates:
     *  Sum: '<S175>/Sum'
     */
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator4_SYSTEM_ = 0U;
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator4_DSTATE = rtb_Merge1;
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator4_PREV_U = rtb_Max + Integrator;
     ...
     *  Product: '<S1>/Product'
     */
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator2_SYSTEM_ = 0U;
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator2_DSTATE = rtb_Merge_ik;

    /* Outputs for Atomic SubSystem: '<S19>/Two inputs CRL' */
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator2_PREV_U = ((real32_T)
      (rtb_Numberofpolepairs * (real_T)rtb_Step * 2666.6666666666665) +
      -1697.8667F * rtb_Merge_ik) - 2666.66675F * z_alpha;
     ...
     *  Product: '<S1>/Product'
     */
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator3_SYSTEM_ = 0U;
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator3_DSTATE = rtb_Add1;

    /* Outputs for Atomic SubSystem: '<S19>/Two inputs CRL' */
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator3_PREV_U = ((real32_T)(wnq *
      (real_T)rtb_Step * 2666.6666666666665) + -1697.8667F * rtb_Add1) -
      2666.66675F * z_beta;

    /* End of Outputs for SubSystem: '<S19>/Two inputs CRL' */

    /* Update for DiscreteIntegrator: '<S14>/Discrete-Time Integrator1' */
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator1_SYSTEM_ = 0U;
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator1_DSTATE = rtb_Add2;
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator1_PREV_U = rtb_DTC;

    /* Update for DiscreteIntegrator: '<S14>/Discrete-Time Integrator' */
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator_SYSTEM_E = 0U;
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator_DSTATE = int_sat_ib;
    Foc_model_Matlab_1_DW.DiscreteTimeIntegrator_PREV_U = rtb_algDD_o1_f;

    /* Update for DiscreteIntegrator: '<S166>/Integrator' */
    Foc_model_Matlab_1_DW.Integrator_SYSTEM_ENABLE = 0U;
    Foc_model_Matlab_1_DW.Integrator_DSTATE = Integrator;
    Foc_model_Matlab_1_DW.Integrator_PREV_U = rtb_Gain_b;

    /* Update for Delay: '<S16>/Delay' */
    Foc_model_Matlab_1_DW.Delay_DSTATE[Foc_model_Matlab_1_DW.CircBufIdx] =
      CurrentControl_ELAPS_T;
    if (Foc_model_Matlab_1_DW.CircBufIdx < 9U) {
      Foc_model_Matlab_1_DW.CircBufIdx++;
    } else {
      Foc_model_Matlab_1_DW.CircBufIdx = 0U;
    }
...
void Foc_model_Matl_SpeedControl(void)
{
  ...
  /* user code (Output function Body) */
  {
    ...
    if ((real32_T)Enable > 0.5F) {
       ...
       *  Switch: '<S8>/Switch'
       */
      Speed_Ref = Foc_model_Matlab_1_B.RT3;
    }
    ...
    /* Constant: '<S8>/Id_ref' */
    Foc_model_Matlab_1_B.Id_ref = 0.0F;
    ...
    if ((!Foc_model_Matlab_1_DW.speed_error_pre_not_empty) ||
        rtb_LogicalOperator1) {
      /* '<S221>:1:9' */
      /* '<S221>:1:10' */
      Foc_model_Matlab_1_DW.speed_error_pre = 0.0F;
      Foc_model_Matlab_1_DW.speed_error_pre_not_empty = true;
    }

    if ((!Foc_model_Matlab_1_DW.N_ref_pre_not_empty) || rtb_LogicalOperator1) {
      /* '<S221>:1:13' */
      /* '<S221>:1:14' */
      Foc_model_Matlab_1_DW.N_ref_pre = 0.0F;
      Foc_model_Matlab_1_DW.N_ref_pre_not_empty = true;
    }

    if ((!Foc_model_Matlab_1_DW.T_n_pre_not_empty) || rtb_LogicalOperator1) {
      /* '<S221>:1:17' */
      /* '<S221>:1:18' */
      Foc_model_Matlab_1_DW.T_n_pre_not_empty = true;
    }

    /* '<S221>:1:21' */
    speed_error = Foc_model_Matlab_1_B.RT3 - Speed_Ref;

    /* '<S221>:1:23' */
    s = Lambda * Foc_model_Matlab_1_DW.speed_error_pre + speed_error;
    ...
    T_eq = (Speed_Ref - Lambda * speed_error) * 1.2E-5F + 1.0E-7F *
      Foc_model_Matlab_1_DW.N_ref_pre;

    /* '<S221>:1:34' */
    /* '<S221>:1:36' */
    Foc_model_Matlab_1_DW.N_ref_pre = Speed_Ref;

    /* '<S221>:1:37' */
    Foc_model_Matlab_1_DW.speed_error_pre = speed_error;

    /* '<S221>:1:38' */
    /* '<S221>:1:42' */
    Foc_model_Matlab_1_B.I_ref = (T_eq + s) / 0.097F;
    if (!(Foc_model_Matlab_1_B.I_ref >= -0.5F)) {
      Foc_model_Matlab_1_B.I_ref = -0.5F;
    }
     ...
     *  Switch: '<S8>/Switch'
     */
    SpeedError = Speed_Ref - Foc_model_Matlab_1_B.RT3;
...
void Foc_model_Matlab_1_step0(void)    /* Sample time: [5.0E-5s, 0.0s] */
{
...
void Foc_model_Matlab_1_step1(void)    /* Sample time: [0.0001s, 0.0s] */
{
  int32_T tmp;
  ...
  tmp = Foc_model_Matlab_1_DW.RT11_ActiveBufIdx << 1;
  Idq_ref_PU[0] = Foc_model_Matlab_1_DW.RT11_Buffer[tmp];
  Idq_ref_PU[1] = Foc_model_Matlab_1_DW.RT11_Buffer[tmp + 1];
  ...
  if (Foc_model_Matlab_1_DW.is_active_c1_Foc_model_Matlab_1 == 0) {
    /* Entry: Hardware Initialization/Enable PDB and start FTM */
    Foc_model_Matlab_1_DW.is_active_c1_Foc_model_Matlab_1 = 1U;

    /* Entry Internal: Hardware Initialization/Enable PDB and start FTM */
    /* Transition: '<S212>:10' */
    Foc_model_Matlab_1_DW.is_c1_Foc_model_Matlab_1 = Foc_model_Matlab_1_IN_A;
  } else if (Foc_model_Matlab_1_DW.is_c1_Foc_model_Matlab_1 ==
             Foc_model_Matlab_1_IN_A) {
    ...
    Foc_model_Matlab_1_DW.is_c1_Foc_model_Matlab_1 = Foc_model_Matlab_1_IN_END;
  } else {
    /* During 'END': '<S212>:39' */
    /* Transition: '<S212>:41' */
    /* Event: '<S212>:36' */
...
void Foc_model_Matlab_1_step2(void)    /* Sample time: [0.001s, 0.0s] */
{
  /* RateTransition: '<Root>/RT2' */
  Speed_Ref_PU = Foc_model_Matlab_1_DW.RT2_Buffer0;

  /* RateTransition: '<Root>/RT1' */
  Speed_fb = SpeedMeasured;

  /* RateTransition: '<Root>/RT3' */
  Foc_model_Matlab_1_B.RT3 = Foc_model_Matlab_1_B.SpeedGain;
  ...
  /* RateTransition: '<Root>/RT11' */
  Foc_model_Matlab_1_DW.RT11_Buffer[(Foc_model_Matlab_1_DW.RT11_ActiveBufIdx ==
    0) << 1] = Foc_model_Matlab_1_B.Id_ref;
  Foc_model_Matlab_1_DW.RT11_Buffer[1 +
    ((Foc_model_Matlab_1_DW.RT11_ActiveBufIdx == 0) << 1)] =
    Foc_model_Matlab_1_B.I_ref;
  Foc_model_Matlab_1_DW.RT11_ActiveBufIdx = (int8_T)
    (Foc_model_Matlab_1_DW.RT11_ActiveBufIdx == 0);
   ...
   * application lifespan selected.
   */
  Foc_model_Matlab_1_M->Timing.clockTick2++;
}

/* Model step function for TID3 */
void Foc_model_Matlab_1_step3(void)    /* Sample time: [0.1s, 0.0s] */
{
   ...
   *  Gain: '<S218>/rpm2PU'
   */
  Foc_model_Matlab_1_DW.RT2_Buffer0 = 0.0005F * DesiredSpeed;

  /* End of Outputs for S-Function (fcgen): '<S3>/SCI_Rx_INT' */
}

/* Model initialize function */
void Foc_model_Matlab_1_initialize(void)
{
  /* Registration code */
...
void Foc_model_Matlab_1_terminate(void)
{
