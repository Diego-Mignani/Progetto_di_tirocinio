...
{
  /* Initialize model */
  Foc_model_Matlab_ultimo_initialize();
}
...
void SYSTEM_TASK(void)
{
   ...
   */
  Foc_model_Matlab_ultimo_SetEventsForThisBaseStep(eventFlags);

  /* Set model inputs associated with base rate here */
  Foc_model_Matlab_ultimo_step(0);

  /* Get model outputs here */
  for (i = 1; i < 4; i++) {
    if (eventFlags[i]) {
      Foc_model_Matlab_ultimo_step(i);
