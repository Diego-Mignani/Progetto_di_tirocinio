...
{
  /* Initialize model */
  mcb_pmsm_foc_hall_S32K144EVB_initialize();
}
...
void SYSTEM_TASK(void)
{
   ...
   */
  mcb_pmsm_foc_hall_S32K144EVB_SetEventsForThisBaseStep(eventFlags);

  /* Set model inputs associated with base rate here */
  mcb_pmsm_foc_hall_S32K144EVB_step(0);

  /* Get model outputs here */
  for (i = 1; i < 3; i++) {
    if (eventFlags[i]) {
      mcb_pmsm_foc_hall_S32K144EVB_step(i);
